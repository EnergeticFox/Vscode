...
class KeyboardActionExecute(threading.Thread):
    ...
    def __init__(self, file_name, execute_count=0):
        super().__init__()
        self.file_name = file_name
...
def command_adapter(action):
    if action == 'listener':
        ...
        if startExecuteBtn['text'] == '开始回放':
            custom_thread_list = [
                {
                    'obj_thread': KeyboardActionExecute(aa,execute_count=playCount.get()),
...
if __name__ == '__main__':
    ...
    aa='win+Prtsc.action'
    startExecuteBtn = tkinter.Button(root, text="开始回放", command=lambda: command_adapter(aa,'execute'))
